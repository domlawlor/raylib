...
void DrawCapsule(Vector3 startPos, Vector3 endPos, float radius, int slices, int rings, Color color)
{
    ...
    if (sphereCase) direction = CLITERAL(Vector3){0.0f, 1.0f, 0.0f};
...
Material LoadMaterialDefault(void)
{
    ...
    material.maps[MATERIAL_MAP_DIFFUSE].texture = CLITERAL(Texture2D){ rlGetTextureIdDefault(), 1, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
...
Mesh GenMeshCylinder(float radius, float height, int slices)
{
    ...
    if (slices >= 3)
    {
        ...
        par_shapes_rotate(cylinder, -PI/2.0f, xaxis);

        // Generate an orientable disk shape (top cap)
        par_shapes_mesh *capTop = par_shapes_create_disk(radius, slices, center, zaxis);
        capTop->tcoords = PAR_MALLOC(float, 2*capTop->npoints);
        for (int i = 0; i < 2*capTop->npoints; i++) capTop->tcoords[i] = 0.0f;

		
        par_shapes_rotate(capTop, -PI/2.0f, xaxis);
        par_shapes_rotate(capTop, 90*DEG2RAD, yaxis);
        par_shapes_translate(capTop, 0, height, 0);

        // Generate an orientable disk shape (bottom cap)
        par_shapes_mesh *capBottom = par_shapes_create_disk(radius, slices, center, negZAxis);
        capBottom->tcoords = PAR_MALLOC(float, 2*capBottom->npoints);
        for (int i = 0; i < 2*capBottom->npoints; i++) capBottom->tcoords[i] = 0.95f;
        par_shapes_rotate(capBottom, PI/2.0f, xaxis);
        par_shapes_rotate(capBottom, -90*DEG2RAD, yaxis);
...
Mesh GenMeshCone(float radius, float height, int slices)
{
    ...
    if (slices >= 3)
    {
        ...
        par_shapes_rotate(cone, -PI/2.0f, xaxis);
        par_shapes_rotate(cone, PI/2.0f, yaxis);
...
BoundingBox GetMeshBoundingBox(Mesh mesh)
{
    ...
    {
        minVertex = CLITERAL(Vector3){ mesh.vertices[0], mesh.vertices[1], mesh.vertices[2] };
        maxVertex = CLITERAL(Vector3){ mesh.vertices[0], mesh.vertices[1], mesh.vertices[2] };

        for (int i = 1; i < mesh.vertexCount; i++)
        {
            minVertex = Vector3Min(minVertex, CLITERAL(Vector3){ mesh.vertices[i*3], mesh.vertices[i*3 + 1], mesh.vertices[i*3 + 2] });
            maxVertex = Vector3Max(maxVertex, CLITERAL(Vector3){ mesh.vertices[i*3], mesh.vertices[i*3 + 1], mesh.vertices[i*3 + 2] });
        }
    }
...
void DrawBillboard(Camera camera, Texture2D texture, Vector3 position, float size, Color tint)
{
    ...
    DrawBillboardRec(camera, texture, source, position, CLITERAL(Vector2){ size, size }, tint);
}
...
static Model LoadOBJ(const char *fileName)
{
    ...
    if (fileText != NULL)
    {
        ...
        for (unsigned int m = 0; m < materialCount; m++)
        {
            ...
            model.materials[m].maps[MATERIAL_MAP_DIFFUSE].texture = CLITERAL(Texture2D){ rlGetTextureIdDefault(), 1, 1, 1, PIXELFORMAT_UNCOMPRESSED_R8G8B8A8 };
            ...
            model.materials[m].maps[MATERIAL_MAP_DIFFUSE].color = CLITERAL(Color){ (unsigned char)(materials[m].diffuse[0]*255.0f), (unsigned char)(materials[m].diffuse[1]*255.0f), (unsigned char)(materials[m].diffuse[2]*255.0f), 255 }; //float diffuse[3];
            ...
            model.materials[m].maps[MATERIAL_MAP_SPECULAR].color = CLITERAL(Color){ (unsigned char)(materials[m].specular[0]*255.0f), (unsigned char)(materials[m].specular[1]*255.0f), (unsigned char)(materials[m].specular[2]*255.0f), 255 }; //float specular[3];
            ...
            model.materials[m].maps[MATERIAL_MAP_EMISSION].color = CLITERAL(Color){ (unsigned char)(materials[m].emission[0]*255.0f), (unsigned char)(materials[m].emission[1]*255.0f), (unsigned char)(materials[m].emission[2]*255.0f), 255 }; //float emission[3];
